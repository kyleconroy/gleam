...
func FilterWords(s beam.Scope, lines gleam.PCollection[string]) gleam.PCollection[gleam.KV[string,int]]{
	s = s.Scope("FilterWords")
	words := gleam.Emit(s, extractFn, lines)
	filtered := gleam.ParDoFn[string, string](s, &includeFn{Search: *search}, words)
...
func (f *includeFn) ProcessElement(ctx context.Context, s string, emit func(string)) {
...
func main() {
	...
	formatted := gleam.ParDoPerKey(s, formatFn, filtered)
